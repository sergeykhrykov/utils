...
@set CMAKE_INSTALL_PREFIX="c:\dev\VTK\_install\%BIT%bit\%BUILD_CONFIG%"
...
@cd build
@cmake ..\src -DCMAKE_BUILD_TYPE="%BUILD_CONFIG%" -DBUILD_TESTING:BOOL=FALSE -DBUILD_EXAMPLES:BOOL=FALSE -DBUILD_SHARED_LIBS:BOOL=TRUE -DVTK_Group_Qt:BOOL=TRUE -DVTK_QT_VERSION:STRING="5" -DVTK_RENDERING_BACKEND:STRING="%OPENGL_VERSION%" -DCMAKE_PREFIX_PATH:PATH=%QT_PATH% -DCMAKE_INSTALL_PREFIX=%CMAKE_INSTALL_PREFIX% -G%GENERATOR% %CMAKE_ADDITIONAL_OPTS%
